...
function buildSharePage(filename, displayName, size, expIso, tok) {
  ...
  const previewUrl = `?tok=${encodeURIComponent(tok)}`;
  ...
  let preview = '';
  if (isImg) {
    preview = `<div class="pv-wrap img-wrap"><img src="${previewUrl}" alt="${nameSafe}" loading="lazy" onerror="this.closest('.pv-wrap').style.display='none'"></div>`;
  } else if (isVid) {
